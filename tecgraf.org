...
#+begin_src bash :results output :tangle ./bin/download_iup-go :tangle-mode (identity #o755) :shebang #!/usr/bin/env bash
  set -ev
  REMOTE_IUP_GO_REPO="https://github.com/gen2brain/iup-go.git"
  . "${TECGRAF_REPO_ROOT}/bin/bashrc_tecgraf"
  . /tmp/target.env
...
** overwrite iup-go
#+begin_src bash :results output :tangle ./bin/overwrite_iup-go :tangle-mode (identity #o755) :shebang #!/usr/bin/env bash
  set -ev
  . "${TECGRAF_REPO_ROOT}/bin/bashrc_tecgraf"
  . /tmp/target.env
